...
def load_glove(filename):
    ...
    f.close()
    gdict = {}
    for line in glovetext.split('\n'):
        wordList = line.split()
        if len(wordList) > 0:
            gdict[wordList[0]] = np.array(wordList[1:]).astype(np.float)
    return gdict
...
def load_articles(articles_dirname, gloves):
    ...
    for file in allfiles:
        ...
        fileinfo = [filename, title, article, centroid]
        allfileinfo.append(fileinfo)
    return allfileinfo
...
def doc2vec(text, gloves):
    ...
    for word in wordlist:
        if word in gloves:
            sumvec = sumvec + gloves[word]
            count += 1
    centroid = sumvec / count
    return centroid
...
def distances(article, articles):
    ...
    """
    articleCent = article[3]
    distances = []
    for a in articles:
        distance = np.linalg.norm(a[3] - articleCent)
        distances.append((distance,a[0]))
...
def recommended(article, articles, n):
    ...
    """
    rec = distances(article, articles)
    recommended = sorted(rec, key=lambda tup: tup[0])
    recommresult = []
    for a in recommended[1:(n + 1)]:
        for art in articles:
            if a[1] == art[0]:
                recommresult.append(art)
    return recommresult
